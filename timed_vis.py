...
uniform highp sampler2D transp_texture;

void main()
{
    vec4 texTransp, texColor, resultColor;
    texColor = texture2D(s_texture, gl_PointCoord);
    texTransp=texture2D(transp_texture,vec2(10,0));
    resultColor = vec4(texColor.x,texColor.y,texColor.z,texTransp);
...
class Canvas(app.Canvas):
    ...
    def __init__(self):
        ...
        a_transp=self.transp[np.random.randint(0,5000),:10000].reshape(10000,1)
        self._program['transp_texture']=gloo.Texture2D(a_transp)
    #Eself._program['a_transp']=a_transp
    ...
    def _new_explosion(self):
        ...
        data['a_position'] = pos
        #data['a_transp']=transp[np.random.randint(0,5000),:].reshape(10000,)
        #print(data['a_transp'])
